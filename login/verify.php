    ...
    if(mysqli_num_rows($resultfrompending) > 0){ //is valid PIN, there are rows
        while($rowfetchpending=mysqli_fetch_array($resultfrompending)){
            ...
            while($rowfetchsignup=mysqli_fetch_array($resultfetchsignup)){
                ...
                echo "You are now a legit user. Click <a href='http://localhost:8080/login/login.php'>here</a> to login.";
            }
        }
    }
    else{
        //echo "invalid";
		$url="http://localhost:8080/login/error.php?error=invalidtoken";
		header('Location:' . $url);
    }
?>
</body>
